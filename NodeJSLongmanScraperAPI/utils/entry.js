...
export async function handleEntry($entry, $, browser, sharedData) {
  //* WORD: sth like: "gotta"
  let word = "";
  ...
  const sensesScrapedData = $senses.toArray().map(async ($sense, i) => {
  ...
  return await Promise.all(sensesScrapedData);
}
